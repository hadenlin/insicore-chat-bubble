(function () {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function sync() {
            ...
            previewFrame.contentWindow.postMessage(payload, (pcbL10n && pcbL10n.siteOrigin) || '*');
        }
        ...
        function extraChannelRows(idx, ch) {
            ...
            if (isForm) {
                ...
                <div class="pcb-detail-row">
                    ...
                    <input type="text" class="pcb-form-title-val" value="${escAttr(fc.title || '')}" placeholder="${escAttr(pcbL10n.defaultFormTitle || 'Send us a message')}">
                </div>
                <div class="pcb-detail-row">
                    <label>${escHtml(pcbL10n.formSubmitLabel || 'Submit Button Text')}</label>
                    <input type="text" class="pcb-form-submit-val" value="${escAttr(fc.submit_label || '')}" placeholder="${escAttr(pcbL10n.defaultFormSubmit || 'Send Message')}">
                </div>
                <div class="pcb-detail-row pcb-checkrow">
                    <label><input type="checkbox" class="pcb-form-phone-val" ${fc.show_phone ? 'checked' : ''}> ${escHtml(pcbL10n.showPhoneField || 'Show Phone field')}</label>
                    <label><input type="checkbox" class="pcb-form-message-val" ${(fc.show_message !== false) ? 'checked' : ''}> ${escHtml(pcbL10n.showMessageField || 'Show Message field')}</label>
                </div>
        ...
        function render() {
            ...
            if (channels.length === 0) {
                stack.innerHTML = `<div class="pcb-stack-empty">
                    ...
                    ${escHtml(pcbL10n.emptyStack || 'Click an app above to add it')}</div>`;
                return;
            }
            ...
            channels.forEach((ch, idx) => {
                ...
                const urlHint = ch.url ? ch.url.replace(/^https?:\/\//, '') : (pcbL10n.noUrlSet || '— no URL set —');
                ...
                item.innerHTML = `
                    <span class="pcb-item-remove-abs pcb-rm" title="${escAttr(pcbL10n.removeTitle || 'Remove')}">
                ...
                if (urlEl) {
                    urlEl.addEventListener('input', (e) => {
                        ...
                        item.querySelector('.pcb-url-hint').textContent =
                            e.target.value ? e.target.value.replace(/^https?:\/\//, '') : (pcbL10n.noUrlSet || '— no URL set —');
                        sync();
                    });
                }
        ...
        document.querySelectorAll('.pcb-app-tile[data-type]').forEach(tile => {
            tile.addEventListener('click', () => {
                ...
                if (type !== 'custom_link' && channels.some(ch => ch.type === type)) {
                    showToast((pcbL10n.alreadyAdded || '%s already added').replace('%s', tile.dataset.label));
        ...
        if (formTile) {
            formTile.addEventListener('click', () => {
                ...
                channels.push({
                    type: 'form', label: (pcbL10n.contactFormLabel || 'Contact Form'), icon: formIcon,
                    color: '#6366f1', hover: '#4f46e5', enabled: true, url: '',
                    form_config: { title: (pcbL10n.defaultFormTitle || 'Send us a message'), submit_label: (pcbL10n.defaultFormSubmit || 'Send Message'), show_phone: true, show_message: true }
                });
                render(); sync();
                document.querySelector('.pcb-tab-btn[data-tab="channels"]')?.click();
            });
